...
class Membership {
  ...
  static async create(userId, teamId) {
    try {
      console.log("Creating membership:", { userId, teamId });
  ...
  static async exists(userId, teamId) {
    try {
      console.log("Checking membership exists:", { userId, teamId });

      const userIdNum = parseInt(userId);
      const teamIdNum = parseInt(teamId);

      if (isNaN(userIdNum) || isNaN(teamIdNum)) {
        console.error("Invalid ID parameters:", { userId, teamId });
        return false;
      }
      ...
      console.log("Membership exists result:", exists);
      return exists;
    } catch (error) {
      console.error("Membership exists check error:", error);
      throw error;
    }
  }
